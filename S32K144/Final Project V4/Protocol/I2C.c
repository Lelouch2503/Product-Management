...
{
	PCC->PCCn[PCC_LPI2C0_INDEX] = ((1 << 30) | (6 << 24));
}
...
uint8_t LPI2C0_writeMulti(uint8_t address, uint8_t reg_address, uint8_t *data, uint8_t n_bytes)
{
    if(bus_busy()) return (error |= (1 << BUSY));

    // Generate start condition and send slave address
    generate_start_ACK(address);

    // Send register address
    transmit_data(reg_address);
...
static void generate_start_ACK(uint8_t address)
{
    uint32_t command    = (uint32_t)(address << 0);
    command             |= (4 << 8);
    LPI2C0->MTDR = command;
}
...
void LPI2C0_Master_IRQHandler(void)
{
    ...
    if(LPI2C0->MSR & (1 << 13))
    {
        ...
        LPI2C0->MCR &= ~(1u << 1);  // RST remains set until cleared by SW.
        LPI2C0_init();
    }
}
